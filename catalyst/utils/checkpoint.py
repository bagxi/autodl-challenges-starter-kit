    ...
    for dict2load, name2load in zip(
        ...
        if isinstance(dict2load, dict):
            for key, value in dict2load.items():
                if value is not None:
                    name2load_ = "{}_{}_state_dict".format(name2load, key) 
                    value.load_state_dict(checkpoint[name2load_])
        else:
            name2load = "{}_state_dict".format(name2load)
...
def save_checkpoint(
    ...
    filename = "{}/{}.pth".format(logdir, suffix)
    torch.save(checkpoint, filename)
    if is_best:
        shutil.copyfile(filename, "{}/best.pth".format(logdir))
    if is_last:
        shutil.copyfile(filename, "{}/last.pth".format(logdir))
    return filename
